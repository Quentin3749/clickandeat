...
@section('content')
    <div class="container"> {{-- Conteneur principal Bootstrap --}}
        ...
        <div class="mb-3">
            <strong>Client:</strong> {{ $order->user ? $order->user->name : 'Utilisateur inconnu' }} {{-- Client avec vérification --}}
        </div>

        <div class="mb-3">
            <strong>Restaurant:</strong> {{ $order->restaurant ? $order->restaurant->name : 'Non spécifié' }} {{-- Restaurant avec vérification --}}
        </div>
        ...
        @if(auth()->check() && auth()->user()->isRestaurateur())
            <a href="{{ url('/restaurateur/commandes/' . $order->id . '/edit') }}" class="btn btn-warning">Modifier la Commande</a> {{-- Lien direct vers l'édition --}}
        @endif
    </div>
@endsection